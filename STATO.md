...
| (Directus UI + API) | **TRANS-FLOW-01 ✅ COMPLETO** | Flow "Import traduzione da JSON" configurato e testato: crea articolo EN pubblicato, link bidirezionale IT↔EN, loop prevention OK. Flow "Esporta per traduzione" (trigger manuale): genera `json_export` nell'articolo IT. Campi `json_export` (readonly→editabile) e `json_traduzione` operativi. Flusso redazione: pulsante export → copia in Claude → incolla JSON tradotto in `json_traduzione` → salva → EN pubblicato. |
