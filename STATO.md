...
| **DIDASCALIE-FASE4 — Smoke test, parziale** | Eseguiti e chiusi: (1) "Esperienze, i campi dell'estate 1977" IT+EN → corretto; (2) i 6 gruppi di foto condivise → invariati, dentro il campione 66/66; (3) `living-the-essential-not-doing-for-but-living-with` (caso `didascalia_en`) → corretto; (4) campione baseline 56 URL (IT+EN random + casi speciali) → 66/66; (5) articoli senza `immagine_copertina` (10 casi) → placeholder invariato; (8) `cf-cache-status` su 3 URL → header assente come atteso (pagine SSR, non passano dalla cache Cloudflare a livello Worker), `cache-control: s-maxage=3600` invariato. **Non eseguiti in questa sessione, richiedono azione su contenuto reale in produzione — lasciati in sospeso per decisione esplicita:** (6) verifica che editare solo la didascalia IT su un articolo già tradotto non la propaghi automaticamente in EN, e che ricliccando "Avvia/aggiorna traduzione" la propaghi correttamente; (7) flusso end-to-end nuovo articolo di test (crea IT → pubblica → traduci → elimina). Ragionamento per cui non sono stati considerati bloccanti: `translate.ts` non è stato toccato da nessuna fase di questo piano, `didascalia_copertina` è già uno dei 6 campi tradotti dal pulsante (confermato nell'audit del piano), e il meccanismo create/update del pulsante è già stato verificato end-to-end il 2026-08-04 (commit `4c2199b2`) prima di questa sessione. |
