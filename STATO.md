...
**Baseline GSC Crawl Stats registrata** (`DECISIONE-STAGING.md` Appendice A11): ~5-6% 404 sui totali di scansione, nessuna anomalia visibile fino all'11/08 — dato pre-incidente (lag GSC 1-2gg), NON un verdetto sull'impatto della Fase 2. **Ricontrollo programmato tra 2-3 giorni (target 2026-08-15/16).**

**Proprietà GSC corretta da guardare, sempre: `https://ombreeluci.it/`** (URL-prefix, senza www) — è l'unica verificata e attiva post-migrazione CF, l'unica a cui il service account ha accesso. Le varianti `www.ombreeluci.it` nello switcher sono residue pre-migrazione: **mai usarle come fonte di diagnosi** (hanno già causato due falsi allarmi in questa sessione). Dettaglio in `DECISIONE-STAGING.md` Appendice A11.

**RISOLTO — calo click/impressioni da giugno: falso allarme.** Riaperta subito (stessa sessione, non rimandata) invece che aspettare il 15-16/08: query diretta su `https://ombreeluci.it/` (proprietà attiva) mostra traffico in **crescita**, non in calo — 20-40 click/giorno a maggio-giugno, fino a 40-66/giorno a inizio agosto. Il calo visto nello screenshot originale veniva da una proprietà GSC diversa e obsoleta (una variante `www.ombreeluci.it` pre-migrazione, mai deverificata ma non più rappresentativa). Dettaglio in `DECISIONE-STAGING.md` Appendice A11. Nessuna azione necessaria.

**SICUREZZA — secret VPS esposti in chat, ruotati.** Durante l'ispezione di `/opt/oel-cms/docker-compose.yml` (in preparazione alla migrazione del token Cloudflare del Flow "Rebuild CF Pages on Publish" a env var), un `cat` di sola lettura ha stampato in chiaro `POSTGRES_PASSWORD`, `SECRET` (firma JWT/sessioni Directus) e `ADMIN_PASSWORD` (login diretto CMS) — inline nel compose file, non in un `.env` esterno come previsto. Rotazione completa eseguita su richiesta esplicita dell'utente: (1) `POSTGRES_PASSWORD`/`DB_PASSWORD` — generato server-side via SSH, applicato con `ALTER USER` reale in Postgres (necessario: la sola env var non ha effetto su un volume già inizializzato) + `docker compose up -d --no-deps directus`; (2) `SECRET` — rigenerato allo stesso modo, invalida le sessioni JWT attive (accettato, nessuna sessione redazionale critica in corso); (3) `ADMIN_PASSWORD` — **non** rotabile via env var per lo stesso motivo (bootstrap-only), rotata invece via `PATCH /users/{id}` su API Directus con `DIRECTUS_TOKEN` (account di servizio con permessi admin, diverso dall'utente admin stesso), password scelta e digitata dall'utente in locale via script PowerShell (`Read-Host -AsSecureString`), mai transitata in chat. Verificato dopo: container `healthy`, ping `200`, `DIRECTUS_TOKEN` statico ancora valido (non dipende da `SECRET`), login admin con nuova password confermato dall'utente. Backup `docker-compose.yml.bak-*` con le vecchie credenziali in chiaro eliminato dal VPS. **Lezione**: `cat`/`grep` di file di configurazione remoti va preceduto da un controllo mirato (`grep` su nomi di variabile, non contenuto completo) quando non è certo che i secret siano esternalizzati in un `.env` — qui l'assunzione (env_file separato) si è rivelata sbagliata ed è costata un'esposizione evitabile.

**SICUREZZA — secondo incidente nella stessa sessione: `LOG_LEVEL=trace` ha esposto un token Directus in chiaro.** Durante la diagnosi del perché il Flow "Rebuild CF Pages on Publish" non generasse mai un deployment reale (vedi sotto), impostato temporaneamente `LOG_LEVEL: "trace"` su Directus per vedere l'errore esatto. A livello `trace`, Directus logga le query SQL complete **con i parametri bindati** — una richiesta autenticata ha stampato in chiaro il valore di un token statico Directus (`select ... where token = $1 [...]`) nell'output di `docker compose logs`, finito sia in chat che in un file di log locale da 430KB. **Non prevedibile dal nome dell'opzione** (non ovvio che "trace" includa bind-parameter delle query SQL) — lezione per il futuro: mai usare `LOG_LEVEL=trace`/`debug` su un'istanza con traffico reale anche per pochi secondi, usare piuttosto un ambiente isolato o operation "Log to Console" mirate nel Flow stesso. Rimosso `LOG_LEVEL` immediatamente, Directus riavviato e verificato sano. File di log locale eliminato. `DIRECTUS_TOKEN` ruotato per precauzione (rigenerato da Directus admin UI dall'utente, mai transitato in chat, `.env` aggiornato) senza prima provare forensicamente quale token fosse — il confronto diretto è stato bloccato dal classificatore di sicurezza di Claude Code stesso (ha riconosciuto il valore come credential-like nel testo del comando), giudicato corretto non aggirarlo: rotazione diretta più semplice e sicura della verifica.
...
**Migrazione secret Flow "Rebuild CF Pages on Publish" completata (2026-08-13)** — token Cloudflare spostato da valore inline nell'operation a variabile d'ambiente `CF_API_TOKEN_OEL` su Directus (docker-compose.yml, VPS), esposta al Flow solo via `FLOWS_ENV_ALLOW_LIST` ristretto a quella singola variabile. Operation aggiornata a `{{$env.CF_API_TOKEN_OEL}}`. Token verificato **valido e funzionante in isolamento** (chiamata diretta all'endpoint Cloudflare, stesso URL/metodo del Flow, ha creato un deployment reale `c3ea6b8f...`).

**Scoperta collaterale, più grande del previsto: il Flow non si attiva mai, indipendentemente dal token.** Cercando conferma che il rebuild partisse da Directus, trovato che **nessun deployment CF Pages storico (controllati gli ultimi 25, arco 2026-08-12/13) ha mai avuto `deployment_trigger.type: "ad_hoc"`** (il tipo che risulterebbe da questo Flow) — solo `github:push` (commit) e `deploy_hook` (meccanismo diverso). Diagnosticato con una operation "Log to Console" temporanea inserita nella catena (`check_published` → checkpoint → `trigger_rebuild`, poi rimossa): **il messaggio di checkpoint non compare mai nei log**, nemmeno per una transizione bozza→pubblicato reale sull'articolo di test (`lanno-della-disabilita-nelle-serie-tv`) — il che sposta la causa dal noto bug della condizione (`$trigger.payload.stato` assente per edit senza toccare `stato`, già documentato sopra) a un livello precedente: **il trigger dell'evento stesso non sembra attivarsi**, non solo la condizione. Non ancora diagnosticato oltre — richiede una sessione dedicata (verificare se il tipo di trigger `action`/scope `items.update` sia corretto per questa versione Directus 11.16.1, o se `accountability: "activity"` interferisca). Questo item si fonde con il punto "decisione + fix condizione Flow rebuild-on-edit" già in backlog: il problema è più ampio di quanto pensato inizialmente.

**Lezione sulla diagnosi stessa**: durante questa indagine, impostare temporaneamente `LOG_LEVEL: "trace"` su Directus per vedere l'errore ha esposto un token in chiaro nei log (vedi voce SICUREZZA sopra) — per diagnosi future su questo Flow, usare SOLO operation "Log to Console" temporanee con messaggi statici non sensibili (tecnica che ha effettivamente funzionato per isolare "il trigger non parte"), mai alzare il log level su un'istanza con traffico reale.

**AGGIORNAMENTO (2026-08-13, stessa sessione, più tardi) — causa esatta trovata.** Diagnosi ripresa con due test mirati sull'articolo "test" (id `1ae92bc1...`, contenuto non reale, stato originale: bozza):

1. **Il trigger scatta sempre** — verificato con una operation "Log to Console" agganciata come vero entry point del Flow (prima della condizione, non dopo, a differenza del test precedente): il checkpoint compare in log per ogni salvataggio, anche uno che non tocca lo stato. Esclude l'ipotesi precedente "il trigger non si attiva mai" — era un artefatto del test di prima (il checkpoint era agganciato DOPO la condizione, quindi misurava la condizione, non il trigger).
2. **La condizione rifiuta anche con payload corretto** — aggiunta un'operation di log sul ramo `reject` di `check_published`, interpolando `{{$trigger.payload.stato}}` nel messaggio. Con l'articolo portato a `published`, il log mostra `REJECT_PAYLOAD_STATO=published__FULLPAYLOAD={"stato":"published"}` — il payload è ESATTAMENTE quello atteso dal filtro (`{"_eq":"published"}`), eppure la condizione prende comunque il ramo reject.

**Causa isolata**: il filtro della condition-operation usa `"$trigger.payload.stato"` come CHIAVE di un oggetto filtro Directus (`{"$trigger.payload.stato":{"_eq":"published"}}`) — questa sintassi NON viene valutata correttamente dal motore filtri delle condition-operation, anche se lo stesso identico path funziona perfettamente come interpolazione mustache (`{{$trigger.payload.stato}}`) in una operation di tipo log. Sembra una differenza specifica di Directus 11.16.1 tra risoluzione dei path `$trigger` nei filtri delle condition vs nel templating — non un problema di trigger, scope, permessi, o payload mancante come ipotizzato in precedenza.

**Confermato: NON è un bug isolato nostro — è un limite noto delle condition-operation Directus**, documentato dalla community (non un changelog/issue chiuso con fix ufficiale, ma un pattern ricorrente con workaround condiviso). [Discussion #15568](https://github.com/directus/directus/discussions/15568): le condition-operation su dati annidati/array nel filtro (`$trigger.payload.X`, `$last[0].X`) sono notoriamente inaffidabili — la sintassi a chiave puntata funziona bene come filtro API standard ma non per validare strutture JSON arbitrarie prodotte da trigger/operation precedenti. Correlato: [Issue #14571](https://github.com/directus/directus/issues/14571) (comportamento inconsistente quando il campo referenziato manca dal payload).

**Fix raccomandato dalla community (verificato funzionante da più utenti nella discussione)**: NON usare `$trigger.payload.stato` direttamente nel filtro della condition. Inserire invece una operation **"Run Script"** tra trigger e condition, che estrae il valore in un campo piatto di primo livello (es. `return { stato: data.$trigger.payload.stato }`), poi far puntare la condition a QUEL campo piatto (es. `{"stato":{"_eq":"published"}}` sull'output dello script) invece di navigare il path annidato. Non implementato in questa sessione (solo diagnosi, come da richiesta) — pattern pronto per la sessione dedicata, non serve reinventarlo da zero.

Il Flow "Pubblicazione programmata" di Matteo (SCHED-PUB-01, commit `56878f4b`, 2026-07-15) usa lo stesso meccanismo `items.update` con `stato=published` per pubblicare via cron — **eredita verosimilmente lo stesso bug, non ne è la causa** (il commit di creazione presuppone esplicitamente che il rebuild "parta automaticamente dal flow event esistente", assunzione mai verificata end-to-end). **Non ancora confermato con Matteo direttamente** — ipotesi dedotta dal codice, non verificata con lui. Da chiedergli esplicitamente: ha notato pubblicazioni programmate che non hanno prodotto un rebuild del sito (pagina categoria/home non aggiornata dopo una pubblicazione schedulata)? Se sì, conferma il bug condiviso; se no, potrebbe esserci una differenza nel suo Flow che vale la pena capire prima di applicare lo stesso fix a entrambi.

Debug operation temporanee create e rimosse pulitamente durante la diagnosi (verificato: il Flow ha di nuovo solo le 2 operation originali, `check_published` → `trigger_rebuild`, nessun residuo). Articolo "test" riportato a bozza.

**INCIDENTE — rotazione `DIRECTUS_TOKEN` ha rotto tutte le pagine articolo in produzione (2026-08-13, ~17:22-17:31 UTC, ~9 minuti).** La rotazione di `DIRECTUS_TOKEN` (fatta per l'esposizione via `LOG_LEVEL=trace`, vedi sopra) ha invalidato il vecchio valore — ma **il sito in produzione ha una copia separata di `DIRECTUS_TOKEN` configurata dentro il progetto Cloudflare Pages** (`ombreeluci-staging`, che nonostante il nome serve realmente `ombreeluci.it` e `www.ombreeluci.it` come custom domain), non letta dal `.env` locale. Le pagine statiche prerenderizzate (home, categoria, archivio) hanno continuato a funzionare; le pagine articolo (SSR, fetch live a Directus) hanno iniziato a rispondere 404 per tutti gli slug, con 401 loggati lato Directus. **Root cause identificata rapidamente** grazie ai log Directus (401 su ogni query articoli) e confermata leggendo gli `env_vars` del progetto Pages via API (elenco identico alle variabili di `.env`, incluso `DIRECTUS_TOKEN` come `secret_text` separato).

**Fix**: aggiornato `DIRECTUS_TOKEN` nel progetto Cloudflare Pages via API (`PATCH .../pages/projects/ombreeluci-staging`) col nuovo valore. **Scoperta aggiuntiva**: le env var di Cloudflare Pages si "cristallizzano" nel deployment al momento della build — aggiornare la configurazione del progetto non corregge retroattivamente il deployment già attivo, serve un nuovo deployment perché il fix prenda effetto. Innescato un nuovo deployment via API (stesso meccanismo usato per il test del Flow poco prima), verificato `deploy:success`, poi verificato con curl diretti (home/categoria/3 articoli → 200) e log Directus (zero 401 residui dopo la propagazione edge, ~90 secondi).

**Lezione principale**: quando un secret è condiviso tra ambiente locale (`.env`) e un ambiente di produzione separato (qui: Cloudflare Pages project settings), la rotazione va sempre estesa a **tutti** i posti dove quel secret è configurato, non solo alla sorgente più comoda da editare. Prima di ruotare un token usato per autenticazione a un servizio esterno (Directus in questo caso), cercare esplicitamente OGNI consumer di quel token (qui: script locali `.env` + Cloudflare Pages env vars + eventuali altri Worker/Flow) invece di assumere un solo punto di configurazione. Aggiungere questo controllo alla checklist di rotazione secret per le prossime volte.

**Chiave SSH orfana rimossa (2026-08-13)**: `authorized_keys` sul VPS aveva una seconda voce etichettata `claude-oel` (stesso nome di quella attiva, fingerprint diverso, nessuna privata corrispondente trovata) — nessuno in redazione l'ha riconosciuta. Rimossa da `authorized_keys` (backup lasciato sul VPS: `~/.ssh/authorized_keys.bak-*`), accesso con la chiave attiva verificato funzionante dopo. Restano solo `claude-oel` (attiva) e `claude-oel-2` (presumibilmente seconda postazione).

**Inventario secret creato (2026-08-13)**: `docs/INVENTARIO-SECRET.md` — mappa completa di ogni secret del progetto e di tutti i posti dove è configurato (VPS, `.env` locale, Cloudflare Pages Production **e** Preview separatamente, GitHub Actions, chiavi SSH). Da consultare PRIMA di ruotare qualunque secret d'ora in poi. **RISOLTO (stessa sessione, poco dopo)**: `DIRECTUS_TOKEN` allineato anche su Cloudflare Pages Preview (verificato con un deployment di retry: articolo+home 200 su `*.pages.dev`) e su GitHub Actions secret (`gh secret set`). Tutti e 5 i posti ora allineati — dettaglio in `docs/INVENTARIO-SECRET.md`. **Scoperta collaterale**: rilanciando `update-snapshot.yml` per verificare, il workflow fallisce — ma non per il token: si blocca prima, su un semplice health-check (`cms.ombreeluci.it/server/ping` → 403), stesso Bot Fight Mode già diagnosticato per lo smoke-post-deploy. Il bug non è nuovo, solo esteso a un secondo workflow non ancora notato prima. Decisione su Bot Fight Mode (disattivare/accettare/upgrade Pro) resta quella già discussa, non urgente.

**Approfondimento (stessa sessione, su richiesta esplicita di verifica) — `update-snapshot.yml` fallisce sistematicamente da mesi, non solo oggi.** Controllate le ultime 15 esecuzioni via `gh run list`: **tutte fallite**, incluse tutte le schedulate settimanali da 2026-05-11 a oggi (100% failure rate sull'intera cronologia disponibile). Causa confermata identica su tre campioni (18/05, 10/08, 13/08): `HTTP 403` su `cms.ombreeluci.it/server/ping` — stesso Bot Fight Mode, attivo su questo workflow da almeno 3 mesi senza che nessuno se ne accorgesse (nessun alert configurato sui fallimenti di questo workflow specifico).

**Impatto reale, verificato non catastrofico**: `src/data/articoli_snapshot.json` è un fallback usato SOLO in fase di build se Directus non risponde (`src/lib/articoli-build.ts`), non serve traffico live. File fermo dal 2026-04-07 (ultimo commit reale), oggi contiene solo **2 voci** invece di migliaia. Directus è sempre stato raggiungibile durante i build reali finora, quindi questo fallback non è mai stato effettivamente esercitato — ma se Directus fosse irraggiungibile durante un build futuro, le pagine prerenderizzate che dipendono da questo snapshot mostrerebbero quasi nessun articolo invece di una lista ragionevolmente completa. Rete di sicurezza silenziosamente vuota da 4 mesi — stesso pattern del CI rotto, stavolta su un dato invece che su un controllo. **Non risolto in questa sessione** — va aggiunto esplicitamente alla lista degli impatti della decisione Bot Fight Mode già in sospeso, non trattato come problema isolato. Trovate anche: 3 chiavi Algolia/altri secret con permessi Production-only assenti da Preview (divergenza silenziosa), una chiave SSH orfana in `authorized_keys` sul VPS, e alcuni secret GitHub Actions (`CF_API_TOKEN`, `CF_ACCOUNT_ID`) senza consumer identificato — dettaglio completo nel file.
