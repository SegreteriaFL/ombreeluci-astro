...
4. **Due bug noti aperti dal cutover, non bloccanti, da riprendere con calma** (dettaglio Appendice A9): (a) 3 delle 7 regole di redirect bare-path portate in Fase 1 (`/categoria/*`, `/archivio/oel-N/` senza `/it/`, `/page/N/`) danno 404 sull'edge reale nonostante la simulazione locale le avesse validate al 100% — causa non ancora identificata; (b) una Cache Rule di zona ("HTML e JSON su ombreeluci.it — cache edge 1h") che prima era inerte (bypassata dal Worker) ora è attiva, e il meccanismo di purge esistente (`/api/revalidate`) invalida solo la pagina del singolo articolo, non le pagine categoria/home che lo elencano — possibile contenuto stale fino a 1h su quelle pagine dopo una modifica.
...
2. **Cache Rule di zona "HTML e JSON su ombreeluci.it — cache edge 1h" si è attivata per la prima volta.** Match: `(http.host eq "ombreeluci.it" or http.host eq "www.ombreeluci.it") and ((http.request.uri.path.extension in {"html" "json"}) or (not http.request.uri.path contains "." and not starts_with(http.request.uri.path, "/api/")))`. Configurata per **ignorare** il `Cache-Control` di origine e imporre 1h di TTL edge — confermato con header reali (`cf-cache-status: HIT`, `Age` di 200-300s su pagine con `Cache-Control: max-age=0, must-revalidate`). Prima era inerte perché il Worker intercettava tutto prima che questa regola potesse applicarsi. Esiste già un meccanismo di purge-on-publish (`src/pages/api/revalidate.ts`, storicamente collegato a un Flow Directus per `POST .../api/revalidate` — verificato nel codice, non riverificato se il Flow è ancora attivo oggi) che purga e "riscalda" la pagina del singolo articolo modificato. **Gap noto**: purga solo l'URL dell'articolo, non le pagine categoria/home/archivio che lo elencano — quelle possono mostrare anteprime non aggiornate fino a 1h. Non causato dal cutover di oggi (era già così nel design, semplicemente non poteva mai attivarsi prima). **Da rivedere con calma**: ridisegnare le Cache Rule e/o il meccanismo di purge tenendo conto della nuova situazione (Worker non più in mezzo) — non oggi, non sotto la pressione del cutover.
