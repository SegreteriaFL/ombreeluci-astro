...
| UX-02 | M | **Mobile: rifinitura round 2** — header, pagine sezioni (diari, autori, archivio, cerca, sostienici, chi-siamo), pagina 404. Round 1 completato (UX-01), ma rimangono dettagli su molte pagine. |
...
| I18N-01 | L | **Sistema i18n globale** — tutte le label UI del sito sono attualmente solo in italiano o hardcoded. Con la sezione `/blog/en` attiva e possibili lingue future, serve un sistema strutturato: `utils/i18n.ts` esiste ma copre solo poche chiavi. Task: audit completo di tutte le stringhe hardcoded IT in ogni pagina/componente, aggiunta chiavi mancanti in i18n, verifica che ogni pagina usi `t(lang, 'chiave')`. Prerequisito per qualsiasi altra lingua. |
